...
<%@ page import="com.musili.projectManagement.model.Task" %>
    ...
    <div class="row">
        ...
        <div class="col-lg-11">
            ...
            <div class="row" style="margin-top: 50px;">
                ...
                <%
                    List<Task> list= (List<Task>) request.getAttribute("allTasks");
                ...
                <table class="table table-hover table-responsive">
                    ...
                    <tr>
                        ...
                        <td><%=tasklist.getMilestone().getMilestoneName()%></td>
                        <td><%=tasklist.getTaskName()%></td>
                        <td><%=tasklist.getTaskDescription()%></td>
                        <td><%=tasklist.getExpectedStartDate()%></td>
                        <td><%=tasklist.getExpectedEndDate()%></td>
                        <td><%=tasklist.getExpectedDuration()%></td>
                        <td ><%=tasklist.getActualStartDate()%></td>
                        <td ><%=tasklist.getActualEndDate()%></td>
                        <td ><%=tasklist.getActualDuration()%></td>
                        <td>
                            <form class="form-signin" action="taskServlet" method="post">
                                <a href="edit_project.jsp?id=<%=tasklist.getId() %>">
                                    <input type="hidden" name="id" value="<%=tasklist.getId()%>">
                                    <input   name="editor" value="editpro" type="hidden">
                                    <button type="submit" name="task" value="editorTask" class="btn btn-default btn-sm">
                        ...
                        <td>
                            <form class="form-signin" action="taskServlet" method="post">
                                <a href="client_dashboard.jsp?id=<%=tasklist.getId() %>">
                                    <input type="hidden" name="id" value="<%=tasklist.getId()%>">
                                    <button type="submit" onclick="window.alert('are you sure you want to delete');"
                                            name="task" value="deleteTask" class="btn btn-danger">Delete</button>
                                </a>
                            </form>
                        </td>
                    </tr>
